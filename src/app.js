...
function scrCourses() {
  ...
  <div class="cfoot">
    ...
    ${canEdit()
      ? `<button class="btn${c.verified ? ' ghost' : ''}" data-act="verifyCourse" data-a="${c.key}">${c.verified ? 'Verified — tap to reopen' : 'Not verified — tap to verify'}</button>`
...
function onClick(e) {
  ...
  switch (act) {
    ...
    case 'verifyCourse': {
      ...
      askPin(cur.verified ? 'Reopen the ' + cur.name + ' card' : 'Verify the ' + cur.name + ' card',
        cur.verified
          ? 'Reopening unlocks par, stroke index and the yardages so the card can be corrected. Enter your PIN.'
          : 'Verifying locks the card so nothing can be changed by accident. Enter your PIN.',
        cur.verified ? 'Reopen card' : 'Verify card',
        () => { store.writeConfig(cf => { cf.courses[a].verified = !cf.courses[a].verified; }); render(); });
      return;
    }
